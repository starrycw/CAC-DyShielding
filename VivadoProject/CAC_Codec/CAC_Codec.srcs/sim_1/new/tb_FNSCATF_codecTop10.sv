...
module tb_FNSCATF_codecTop10(
    ...
    // 10-bit codec core
    reg [9 : 0] tsv_10;
    reg [`VH_FNSCATF_DataInBitWidth_10bitCW - 1 : 0] datain_10;
    reg [`VH_FNSCATF_DataInBitWidth_10bitCW - 1 : 0] dataout_10;
    assign tsvTrans = tsvLast ^ tsv_10;
    FNSCATF_encoder_top_10 encoder_instance10 (
        .clk(clk_encoder),
        .rst_n(rst_n),
        .datain(datain_10),
        .codeword_regs(tsv_10)
    );

    FNSCATF_decoder_top_10 decoder_instance10 (
        .clk(clk_decoder),
        .rst_n(rst_n),
        .codeword_in(tsv_10),
        .data_reg(dataout_10)
    ...
    initial begin
        ...
        for (cnt_i = 0; cnt_i < N_SIMU_CYCLE; cnt_i++) begin: for_cnt_i
            ...
            clk_decoder <= 0;
            tsvLast <= tsv_10;
            datain_10 = {$random} % (2**(`VH_FNSCATF_DataInBitWidth_10bitCW));
            ...
            clk_decoder <= 1;
            #1;
            if (dataout_10 != datain_10) begin
                cnt_err = cnt_err + 1;
                $display("CODEC10-%d-ERROR: %d -> %b -> %d", cnt_i, datain_10, tsv_10, dataout_10);
            end
            else begin
                cnt_pass = cnt_pass + 1;
                $display("CODEC10-%d-PASS: %d -> %b -> %d", cnt_i, datain_10, tsv_10, dataout_10);
            end
            for (idx_i = 0; idx_i < 9; idx_i++) begin
                ...
                if ( (tsvTrans[9] != 0) && (tsvTrans[0] != 0) ) begin
                    $display("---ERROR! CAC rule violated!---");
                    cnt_violate = cnt_violate + 1;
                    $finish();
                end
