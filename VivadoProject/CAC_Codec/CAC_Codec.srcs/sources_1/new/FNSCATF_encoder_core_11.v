...
module FNSCATF_encoder_core_11(
    ...
    FNSCATF_encoderModule_cmp #(.RES_IN_WIDTH(`VH_FNSCATF_DataInBitWidth_11bitCW), .RES_OUT_WIDTH(`VH_FNSCATF_NSValueMaxBinWidth_P10), .NS_VALUE(`VH_FNSCATF_NSValue_P10)) 
        cmp_module10 (
            .res_in(datain),
            .lock_in(1'b0),
            .q_out(q_out[10]),
    ...
    // Shift
    assign codeout[10] = q_out[10];
    assign codeout[9 : 1] = (q_out[10] == 1'b0)? (q_out[9 : 1]) : (q_out[8 : 0]);
    assign codeout[0] = (q_out[10] == 1'b0)? (q_out[0]) : (1'b0);
