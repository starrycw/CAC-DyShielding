...
module FNSCATF_encoder_core_5(
    input wire [`VH_FNSCATF_DataInBitWidth_5bitCW - 1 : 0] datain,
    ...
    FNSCATF_encoderModule_cmp #(.RES_IN_WIDTH(`VH_FNSCATF_DataInBitWidth_5bitCW), .RES_OUT_WIDTH(`VH_FNSCATF_NSValueMaxBinWidth_P4), .NS_VALUE(`VH_FNSCATF_NSValue_P4)) 
        cmp_module4 (
            .res_in(datain),
            .lock_in(1'b0),
            .q_out(q_out[4]),
    ...
    // Shift
    assign codeout[4] = q_out[4];
    assign codeout[3 : 1] = (q_out[4] == 1'b0)? (q_out[3 : 1]) : (q_out[2 : 0]);
    assign codeout[0] = (q_out[4] == 1'b0)? (q_out[0]) : (1'b0);
